...
const envars = {
  GATE_COGNITO_POOLID_DEV: '',
  GATE_COGNITO_CLIENTID_DEV: '',
  GATE_RECV_DOMAIN: '',
  GATE_RECV_QUEUE_URL: '',
...
const STAGE = 'dev';
const queueUrl = envars.GATE_RECV_QUEUE_URL;
const userPoolId = envars.GATE_COGNITO_POOLID_DEV;
const defaultUserGroup = envars.GATE_DEFAULT_USER_GROUP;
const tableUsers = `${envars.GATE_TABLE_USERS}-${STAGE.toUpperCase()}`;
...
Amplify.configure({
  Auth: {
    region: 'us-east-1',
    userPoolId: envars.GATE_COGNITO_POOLID_DEV,
    userPoolWebClientId: envars.GATE_COGNITO_CLIENTID_DEV,
  },
});

const password = '123paSSword$';
const email = `tester+${v4()}@${envars.GATE_RECV_DOMAIN}`;
let username = '';
